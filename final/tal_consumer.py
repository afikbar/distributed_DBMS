...
def msg_cleaner(msg):
    result = re.sub("[()']",'',msg.decode('utf8')).split(",")
...
def data_filter(msg_dict):#returns true if data is ok
    return (msg_dict['location_raw'].title() 
                in ['San Diego', 'Redding', 'Buttonwillow', 'Modesto']) and
...
def tal_consumer(ip, port, topic):
    ...
    for msg in consumer:
        ...
        if msg_dict['search_conducted'] 
            and msg_dict['stop_date'] >= datetime(2016,1,30):
